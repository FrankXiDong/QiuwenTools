...
async function main() {
    ...
    let pageList = await bot.request({
        ...
        "formatversion": "2",
        "cmtitle": "Category:中国国民党大陆时期党员", // 在此处修改需要清理、批量移动的母分类
        "cmlimit": "max"
    });
    ...
    for (let categoryName0 of categoryList) {
        ...
        // 检查与"中国国民党党员（xxxx年入党）"同年的"xxxx年加入中国国民党的人物"分类是否存在，如不存在则移动至该新分类
        
        // 提取年份模式：匹配 "Category:中国国民党党员（xxxx年入党）" 格式
        const yearPattern = /^Category:中国国民党党员[（(](\d{4})年入党[）)]$/;
        const match = categoryName0.match(yearPattern);
        
        if (match) {
            const year = match[1];
            const targetCategory = `Category:${year}年加入中国国民党的人物`;
            ...
            try {
                ...
                const summary = `机器人：批量移动分类（参见[[Qiuwen_talk:茶馆/存档/编辑/2026年04月#关于Category:依入党年份分类的中国共产党党员|此前讨论]]）（task2-2）"`;
