...
// 从页面内容中移除Wayback模板
function removeWaybackTemplate(content) {
    ...
    // 匹配各种形式的Wayback模板（不区分大小写）
    // 支持: {{Wayback|...}}, {{wayback|...}}, {{WAYBACK|...}} 等
    // 支持带参数的情况: {{Wayback|url=xxx|date=xxx}}
    // 支持多行情况
    
    // 正则表达式说明：
    // \{\{           - 匹配 {{
    // [Ww][Aa][Yy][Bb][Aa][Cc][Kk] - 匹配 Wayback（不区分大小写）
    // [^}]*          - 匹配任意非}字符（包括参数）
    // \}\}           - 匹配 }}
    ...
    do {
        ...
        newContent = newContent.replace(waybackPattern, '');
...
async function main() {
    ...
    // 2. 查询Wayback模板的链入页面
    console.log(pc.cyan('[INFO] 正在查询Wayback模板的链入页面...'));
    ...
    do {
        ...
        try {
            ...
            await logError(bot, `查询Wayback模板链入页面失败`, {
                error: error.message,
                stack: error.stack
            });
            break;
    ...
    console.log(pc.green(`[COMPLETE] 共找到 ${allPages.length} 个包含Wayback模板的页面`));
    
    if (allPages.length === 0) {
        console.log(pc.yellow('[INFO] 没有找到包含Wayback模板的页面，任务结束'));
        return;
    }
    ...
    // 3. 逐个页面处理，移除Wayback模板
    console.log(pc.cyan(`[INFO] 开始逐个页面移除Wayback模板（共 ${pagesToProcess.length} 个页面）...`));
    ...
    for (let i = 0; i < pagesToProcess.length; i++) {
        ...
        try {
            ...
            // 移除Wayback模板
            const { content: newContent, removed } = removeWaybackTemplate(originalContent);
            
            if (!removed) {
                console.log(pc.yellow(`${progress} [SKIP] 未检测到Wayback模板，跳过`));
                skipCount++;
                await sleep(sleepTime);
                continue;
            }
            ...
            try {
                ...
                console.log(pc.green(`${progress} [SUCCESS] 已成功移除Wayback模板`));
                successCount++;
